...
;(function ($) {
    ...
    function get_region_parameters(obj) {
        ...
        // }
        // else {
        //   throw new Error('bbox not valid')
        // }
    }
        ...
        return $.ajax({
            ...
            .then(function (response) {
                ...
                if (obj.options.frequency === 'annual') {
                    data = flatten_and_average(response.data);
                    ...
                    flatten_and_average(response.data).forEach(function (record) {
                        // console.log(response)

                        data[record[0].slice(-2)].push(parseFloat(record[1]));
                    });
    ...
    function flatten_and_average(records) {
        var return_data = [];
        var year;
        _.forEach(records, function (record) {
                mean = _.mean(_.filter(_.flattenDeep(record[1]), function (v) {
    ...
    var climate_widget_graph = function (orig_options) {
        ...
        obj.update = function (delta) {
            ...
            if (obj.options.frequency !== old_options.frequency ||
                ...
                obj.options.bbox !== old_options.bbox ||
